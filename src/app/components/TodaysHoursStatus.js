...
import { getTodayName } from '../../utils/shared';

export default function TodaysHoursStatus({ locationHours, locationUrl, locationName }) {
  if (!locationHours) {
    return (
      <a className="flex flex-row items-center gap-1 text-sm" href={locationUrl} target="_blank" rel="noopener noreferrer">
        <svg xmlns="http://www.w3.org/2000/svg" className="w-3 feather feather-circle fill-gray-600 mt-1 shrink-0" viewBox="0 0 24 24" stroke="smoke" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/></svg>
      ...
      <span className="flex flex-row items-start gap-1 text-sm my-2 cursor-pointer">
        <span>
          ...
          <span className="flex flex-row items-start gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 mt-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
